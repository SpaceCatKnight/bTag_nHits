...
def tagged_jets_hist(datalist,discriminant, dR, discriminant_cut, CSV_cut, bins, Difference=False, mode="pT_hadron",Save=False):
	"""creates a histogram for each dataset given as list of tuples (data, title) of all the jets that were b-tagged by passing a given cut value for CSV and a given discriminant versus a feature given as string to 'mode' (see FEatureDict). The histograms are saved to a root file for further use."""
...
if __name__ == '__main__':
	...
	BG_hadron_DiffHist = BG_hadron_file_diff.Get("Background_Discriminant")
	BG_hadron_file_ratio = rt.TFile("histogram_files/pT_hists/tagged_jets_vs_pT_hadron_dR_0.1_BackgroundL4_L1.root","READ")
	BG_hadron_RatioHist = BG_hadron_file_ratio.Get("Background_Discriminant")
	...
	Efficiency_vs_pT("Mistag_Rate",[(BG_hadron_DiffHist,"L4-L1"),(BG_hadron_RatioHist,"L4/L1"),(BG_hadron_CSVHist,"CSV")], BG_hadron_AllJetsHist, "pT_hadron",Save=True)
	Efficiency_vs_pT("Mistag_Rate_rebin",[(BG_hadron_DiffHist2,"L4-L1"),(BG_hadron_RatioHist2,"L4/L1"),(BG_hadron_CSVHist2,"CSV")], BG_hadron_AllJetsHist2, "pT_hadron",Save=True)

	BG_jet_file_diff = rt.TFile("histogram_files/pT_hists/tagged_jets_vs_pT_jet_dR_0.04_BackgroundL4-L1.root","READ")
	BG_jet_AllJetsHist = BG_jet_file_diff.Get("Background_AllJets")
	BG_jet_CSVHist = BG_jet_file_diff.Get("Background_CSV")
	BG_jet_DiffHist = BG_jet_file_diff.Get("Background_Discriminant")
	BG_jet_file_ratio = rt.TFile("histogram_files/pT_hists/tagged_jets_vs_pT_jet_dR_0.1_BackgroundL4_L1.root","READ")
	BG_jet_RatioHist = BG_jet_file_ratio.Get("Background_Discriminant")
	...
	Efficiency_vs_pT("Mistag_Rate",[(BG_jet_DiffHist,"L4-L1"),(BG_jet_RatioHist,"L4/L1"),(BG_jet_CSVHist,"CSV")], BG_jet_AllJetsHist, "pT_jet",Save=True)
	...
	for n in range(1,10):
		try:
			ClusterMatch('BG'+str(n),Additional_Background_String.format(n), dR, MomentumThresholdBackground,JetMode=True, HadronsNotQuarks=True, BG=True, Plot=False, Axes=None, Save=True, dR_dist = False, LayerHist=False,LightVersion=True, EarlyBreak=180000)
		except:
			continue
	'''
	dR = 0.1
