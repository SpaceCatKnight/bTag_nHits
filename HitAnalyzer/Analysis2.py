...
def QuarkHadronComparison(file_path, title, dR, MomentumThreshold, BG=False, EarlyBreak=0,):
	print "working on file", file_path
	file = rt.TFile(file_path,"READ")
        # open tree file
        tree = file.Get("demo/tree")
	...
	Histogramize([(PT_DIFF,"pT_diff_"+title)], (0,700), "pt_diff", "pt_diff", "# particles", Save=False,Normalize=False, t_sleep=20)
	Histogramize([(ETA_DIFF,"eta_diff_"+title)], (0,0.2), "eta_diff", "eta_diff", "# particles", Save=False,Normalize=False, t_sleep=20)
	Histogramize([(PHI_DIFF,"phi_diff"+title)], (0,0.2), "phi_diff", "phi_diff", "# particles", Save=False,Normalize=False, t_sleep=20)
...
#MatchedParticleHistogram(Signal_file1, dR, MomentumThreshold, Histogram=R_Hist_signal1, BG=False, Save=False, EarlyBreak=0, Flag="Signal1", Mode="decay_vx")
#MatchedParticleHistogram(Signal_file2, dR, MomentumThreshold, Histogram=R_Hist_signal2, BG=False, Save=False, EarlyBreak=0, Flag="Signal2", Mode="decay_vx")
MatchedParticleHistogram(BG_file, dR, MomentumThreshold, Histogram=R_Hist_BG, BG=True, Save=True, EarlyBreak=180000, Flag="BG", Mode="decay_vx")
...
#QuarkHadronComparison(Signal_file1,"2TeV-signal", dR, MomentumThreshold, BG=False, EarlyBreak=2000)
